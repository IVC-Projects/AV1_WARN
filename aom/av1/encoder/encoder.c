...
static int encode_frame_to_data_rate(AV1_COMP *cpi, size_t *size, uint8_t *dest,
  ...
  int cnn_flag = 0; //This flag is set to 1 when the frame being encoded currently is selected into the neural network.
  unsigned int selected_frame_in_gop = cm->current_frame.order_hint % 16;
  /*Intra*/
  /*if (cm->current_frame.frame_type == KEY_FRAME) {
	  cnn_flag = 1;
  }*/
  /*Inter*/
  if (cm->current_frame.frame_type == KEY_FRAME || selected_frame_in_gop == 2 || selected_frame_in_gop == 6 || selected_frame_in_gop == 10 || selected_frame_in_gop == 14 
	  || get_current_frame_ref_type(cpi) == OVERLAY_FRAME) {
	  cnn_flag = 1;
  }
  else {// Go to previous loop filter
	  cm->seq_params.enable_cdef = 1;
	  cm->seq_params.enable_restoration = 1;
  }
  ...
  if (!cm->allow_intrabc) {
    ...
    if (cnn_flag) {
      if (get_current_frame_ref_type(cpi) == OVERLAY_FRAME)  
        additionHandle_blocks(cpi, cm, KEY_FRAME);
      else additionHandle_blocks(cpi, cm, cm->current_frame.frame_type);
    }
	

	/*uint8_t *write_y_buffer = cm->cur_frame->buf.y_buffer;
	FILE *ff = fopen("BQMall_832x480_qp53_deblock.yuv", "ab");
	for (int i = 0; i < cm->cur_frame->buf.y_height; i++) {
		for (int j = 0; j < cm->cur_frame->buf.y_width; j++) {
			fwrite(write_y_buffer + j, 1, 1, ff);
		}
		write_y_buffer += cm->cur_frame->buf.y_stride;
	}
	uint8_t *write_u_buffer = cm->cur_frame->buf.u_buffer;
	for (int i = 0; i < cm->cur_frame->buf.uv_height; i++) {
		for (int j = 0; j < cm->cur_frame->buf.uv_width; j++) {
			fwrite(write_u_buffer + j, 1, 1, ff);
		}
		write_u_buffer += cm->cur_frame->buf.uv_stride;
	}
	uint8_t *write_v_buffer = cm->cur_frame->buf.v_buffer;
	for (int i = 0; i < cm->cur_frame->buf.uv_height; i++) {
		for (int j = 0; j < cm->cur_frame->buf.uv_width; j++) {
			fwrite(write_v_buffer + j, 1, 1, ff);
		}
		write_v_buffer += cm->cur_frame->buf.uv_stride;
	}
	fclose(ff);*/
  } else {
  ...
  cm->seq_params.enable_restoration = 0;
