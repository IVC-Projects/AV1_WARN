...
def prepare_nn_data(train_list, idx_img=None):
    ...
    lowData_list = np.resize(lowData_list, (BATCH_SIZE * IMAGE_BATCH, PATCH_SIZE[0], PATCH_SIZE[1], 1))
    gt_list = np.resize(gt_list, (BATCH_SIZE * IMAGE_BATCH, PATCH_SIZE[0], PATCH_SIZE[1], 1))
...
def getWH(yuvfileName):
    ...
    wxh = os.path.basename(deyuvFilename).split('_')[-2]   #evaluate
...
def save_test_img(inputY, inputCbCr, path):
    ...
    #img = Image.fromarray(inputCbCr, mode='L')
    img = img.convert('RGB')
    img.save(path,'.yuv')
...
def getCbCrdata(path, size):
    w= size[0]
    h=size[1]/2
    #print(w,h)
    skipbyte = size[0] *size[1]
    cc = np.zeros([h, w], dtype="uint8", order='C')
    with open(path, 'rb') as fp:
        fp.seek(skipbyte, 0)
        cc = fp.read()
        #tem = Image.frombytes('L', [w, h], cc)

        cc = np.asarray(tem, dtype='float32')
